...
function OverviewTab() {
  const last12Months = monthlyFrequency.slice(-24)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <ChartContainer config={freqConfig} className="h-48 w-full">
            <BarChart data={last12Months} barCategoryGap="20%">
              ...
              <li key={`${s.date}|${s.name}`} className="py-3 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5 truncate">
                    {s.exercises.slice(0, 4).join(' · ')}{s.exercises.length > 4 ? ` +${s.exercises.length - 4}` : ''}
                  </p>
                </div>
...
function BigLiftsTab() {
  ...
  // Combined chart data — all key lifts on one chart
  /* Big 3 snapshot cards */
  ...
  // For cleanliness only show monthly data points (first occurrence per month)
  const monthlyPoints = (() => {
  ...
  const combinedData = monthlyPoints.map((date) => {
    ...
    for (const ex of KEY_EXERCISES) {
      ...
      // find closest (≤ date)
      const point = [...(history ?? [])].reverse().find((p) => p.date <= date)
  ...
  // Per-lift individual charts (more detailed)
  return (
    <div className="space-y-6">
      {/* Big 3 snapshot */}
      <div className="grid grid-cols-3 gap-4">
        {big3.map((ex) => {
          ...
          return (
            <Card key={ex}>
              <CardHeader className="pb-1">
                <CardDescription style={{ color: liftConfig[key].color }}>{liftConfig[key].label}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-1">
                <p className="text-2xl font-bold">{latest?.e1RM ?? '—'} <span className="text-sm font-normal text-muted-foreground">lbs e1RM</span></p>
                <p className="text-xs text-muted-foreground">PR: {pr?.bestE1RM} lbs ({pr?.bestWeight}×{pr?.bestReps})</p>
              </CardContent>
            </Card>
          )
        })}
      ...
      <Card>
        ...
        <CardContent>
          <ChartContainer config={liftConfig} className="h-72 w-full">
            <LineChart data={combinedData}>
              ...
              <XAxis
                ...
                tickLine={false}
                axisLine={false}
                tickFormatter={(v) => new Date(v + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', year: '2-digit' })}
                interval={Math.floor(combinedData.length / 8)}
              />
              <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} width={40} unit=" lbs" />
              <ChartTooltip content={<ChartTooltipContent />} />
      ...
      <Card>
              ...
              {KEY_EXERCISES.map((ex) => {
                ...
                return (
                  <button
                    ...
                    className={`text-xs px-3 py-1 rounded-full border transition-colors ${active ? 'text-white border-transparent' : 'text-muted-foreground border-border hover:border-foreground/30'}`}
                    style={active ? { backgroundColor: liftConfig[key].color, borderColor: liftConfig[key].color } : {}}
                  >
        ...
        <CardContent className="space-y-6">
          {/* e1RM dots chart */}
          <ChartContainer config={{ e1RM: { label: 'e1RM (lbs)', color: liftConfig[LIFT_KEY[selected]].color } }} className="h-56 w-full">
            <LineChart data={exerciseHistory[selected]}>
              ...
              <XAxis
                ...
                axisLine={false}
                tickFormatter={(v) => new Date(v + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', year: '2-digit' })}
                interval={Math.floor((exerciseHistory[selected]?.length ?? 1) / 8)}
              />
              <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} width={40} unit=" lbs" />
              <ChartTooltip
                content={<ChartTooltipContent formatter={(val, name) => [`${val} lbs`, name === 'e1RM' ? 'e1RM' : name]} />}
              />
              <Line
                type="monotone"
                dataKey="e1RM"
          ...
          <div>
            ...
            <ChartContainer config={repsConfig} className="h-40 w-full">
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          <ChartContainer config={volConfig} className="h-64 w-full">
            <BarChart data={last52} barCategoryGap="10%">
              ...
              />
              <ChartTooltip
                content={<ChartTooltipContent formatter={(val) => [`${Number(val).toLocaleString()} lbs`, 'Volume']} />}
              />
        ...
        <CardContent>
          ...
          >
            <BarChart data={last52} barCategoryGap="10%">
              ...
              <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} width={24} />
              <ChartTooltip content={<ChartTooltipContent />} />
              <ReferenceLine y={3} stroke="#6b7280" strokeDasharray="4 2" label={{ value: 'goal', fontSize: 10, fill: '#6b7280' }} />
            ...
            <tbody className="divide-y divide-border">
              {top.map((pr, i) => (
                <tr key={pr.exercise} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="py-2 text-right text-muted-foreground">{fmtDate(pr.bestDate)}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <StatCard title="Best Set" value={summary.bestSet} sub={summary.bestE1RM ? `${summary.bestE1RM} lbs e1RM` : undefined} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={volConfig} className="h-48 w-full">
              <BarChart data={last20} barCategoryGap="15%">
                ...
                <ChartTooltip content={<ChartTooltipContent formatter={(v) => [`${Number(v).toLocaleString()} lbs`, 'Volume']} />} />
                <Bar dataKey="volume" fill="var(--color-volume)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ChartContainer>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={wtConfig} className="h-48 w-full">
              <LineChart data={sessionData}>
                <CartesianGrid vertical={false} stroke="var(--border)" />
                <XAxis dataKey="date" tick={{ fontSize: 10 }} tickLine={false} axisLine={false}
                  tickFormatter={(v) => new Date(v + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', year: '2-digit' })}
                  interval={Math.floor(sessionData.length / 6)} />
                <YAxis tick={{ fontSize: 10 }} tickLine={false} axisLine={false} width={40} unit=" lbs" />
                <ChartTooltip content={<ChartTooltipContent />} />
                <Line type="monotone" dataKey="maxWeight" stroke="var(--color-maxWeight)" strokeWidth={2} dot={{ r: 2 }} />
              </LineChart>
            </ChartContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Training frequency & volume load</CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer config={{ sets: { label: 'Sets', color: '#818cf8' } }} className="h-48 w-full">
              <BarChart data={last20} barCategoryGap="15%">
                <CartesianGrid vertical={false} stroke="var(--border)" />
                <XAxis dataKey="date" tick={{ fontSize: 10 }} tickLine={false} axisLine={false}
                  tickFormatter={(v) => new Date(v + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer config={repsCfg} className="h-48 w-full">
              <BarChart data={repsByWeight} barCategoryGap="15%">
                <CartesianGrid vertical={false} stroke="var(--border)" />
                <XAxis dataKey="weight" tick={{ fontSize: 10 }} tickLine={false} axisLine={false} unit=" lbs" />
                <YAxis tick={{ fontSize: 10 }} tickLine={false} axisLine={false} width={24} />
          ...
          {[...sessions].reverse().map((s) => (
            <li key={`${s.date}|${s.name}`} className="py-3 flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground mt-0.5 truncate">
                  {s.exercises.slice(0, 5).join(' · ')}{s.exercises.length > 5 ? ` +${s.exercises.length - 5}` : ''}
                </p>
                <div className="flex gap-2 mt-1">
                  ...
                  <Badge variant="outline" className="text-[10px] h-4">
                    {fmtVolume(Math.round(s.totalVolume))} lbs
                  </Badge>
                </div>
              </div>
              <div className="text-right shrink-0">
